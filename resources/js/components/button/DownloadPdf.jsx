 ...
 */
export default function DownloadPdf({ target, fileName = 'document.pdf', className = '', ...props }) {
    ...
    const download = async () => {
        ...
        try {
            ...
            const image = await toPng(element, { pixelRatio: 2, backgroundColor: '#ffffff' });
            ...
            for (let offset = 0; offset < imageHeight; offset += contentHeight) {
                if (offset > 0) {
                    pdf.addPage();
                }
                pdf.addImage(image, 'PNG', PAGE_MARGIN_MM, PAGE_MARGIN_MM - offset, contentWidth, imageHeight);
                // Mask the slices that spill into the top and bottom margins.
                pdf.setFillColor(255, 255, 255);
                pdf.rect(0, 0, pageWidth, PAGE_MARGIN_MM, 'F');
                pdf.rect(0, pageHeight - PAGE_MARGIN_MM, pageWidth, PAGE_MARGIN_MM, 'F');
            }
